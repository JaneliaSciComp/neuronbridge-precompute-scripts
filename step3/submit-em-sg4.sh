...
# start the jobs that run the gradient adjustment in parallel, EM (masks) vs Split Gal 4 (libraries)
# this is a modified version of submitGABatch.sh in the main repo

# global parameters first, then common parameters for this step
source "$(dirname ${BASH_SOURCE[0]})/../global-cdsparams.sh"
source "$(dirname ${BASH_SOURCE[0]})/cdsparams-em-sg4.sh"
